...
class ProcessApiBatch implements ShouldQueue
{
     ...
     */
    public function __construct($startPage, $endPage, $perPage, $minutes, $baseUrl)
    {
    ...
    public function handle(): void
    {
        for ($page = $this->startPage; $page <= $this->endPage; $page++) {
            $apiUrl = "{$this->baseUrl}?minutes={$this->minutes}&page={$page}&per_page={$this->perPage}";
            \Log::info("Fetching data for page {$page}, URL: {$apiUrl}");

            $response = Http::withHeaders(['x-api-key' => env('CAR_API_KEY')])
                ->timeout(120)
                ->retry(3, 1000)
                ->get($apiUrl);
            ...
            if ($response->successful()) {
                ...
                if (!empty($data)) {
                    ...
                    \Log::info("Processed page {$page} successfully.");
                }
            } else {
