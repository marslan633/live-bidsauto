...
class ProcessApiData extends Command
{
    ...
    public function handle()
    {
        ...
        $minutes = 50; // Time frame in minutes
        ...
        try {
            ...
            if ($response->successful()) {
                ...
                if ($totalRecords > 0) {
                    ...
                    $batchSize = 10; // Number of pages per batch
                    ...
                    for ($startPage = 1; $startPage <= $totalPages; $startPage += $batchSize) {
                        ...
                        ProcessApiBatch::dispatch($startPage, $endPage, $perPage, $minutes, $baseUrl);
                        $this->info("Dispatched batch: {$startPage}-{$endPage}");
                    }
                } else {
