...
class ProcessCachedDataToDatabasesWithoutQueueLocal extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'process:process-cached-data-to-databases-without-queue-local';
    ...
    public function handle()
    {
        ...
        collect($data)->chunk(100)->each(function ($chunk) {
            foreach ($chunk as $item) {
                ...
                Log::info('Cachekey', ['cache_key' => $cacheKey]);
        ...
        if($cronRun){
            ...
            $cronRun = CronRunHistory::where('_id', $cronRun)->update([
                'end_time' => Carbon::now(),
                'status' => 'success',
                'updated_at' => now(),
            ]);
    ...
    public function insertBatch(array $batchData, $cacheKey)
    {
        ...
        try {
            ...
            VehicleProcessCachedApiData::where('_id', $cacheKey)->delete();
