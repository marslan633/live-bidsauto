...
class VehicleController extends Controller
{
    ...
    public function filterAttributes(Request $request)
    {
        try {
            ...
            if ($request->has('buy_now')) {
                $buyNowIds = BuyNow::whereIn('name', $request->boolean('buy_now') ? ['buyNowWithPrice'] : ['buyNowWithPrice', 'buyNowWithoutPrice'])->pluck('id');
            ...
            if ($request->has('odometer_min') && $request->has('odometer_max')) {
                $query->whereBetween('odometer_mi', [(int) str_replace(',', '', $request->input('odometer_min')), (int) str_replace(',', '', $request->input('odometer_max'))]);
            }

            if ($request->has('auction_date')) {
                // Handle auction date filtering here
                $auctionDateInput = $request->input('auction_date');
                if (is_array($auctionDateInput) && count($auctionDateInput) === 2) {
                    [$auctionDateFrom, $auctionDateTo] = $auctionDateInput;
                    $query->whereBetween('sale_date', [Carbon::parse($auctionDateFrom)->format('Y-m-d'), Carbon::parse($auctionDateTo)->format('Y-m-d')]);
                }
            }

            // Apply the active filter logic
            if ($activeFilterKey) {
                $filterDetails = $filters[$activeFilterKey];

                // Group by the filtered column and get the count of occurrences
                $filterQuery = $query->selectRaw("{$filterDetails['column']} as id, COUNT(*) as count")
                    ->groupBy($filterDetails['column']);

                // Apply pagination if needed
                $results = $filterQuery->paginate($perPage, ['*'], 'page', $page);

                // Eager load related data to avoid N+1 problem
                $relatedNames = DB::table($filterDetails['table'])->whereIn('id', $results->pluck('id'))->pluck('name', 'id');

                // Map results to the response structure
                $response[$activeFilterKey] = $results->map(function ($item) use ($relatedNames) {
                    return [
                        'id' => $item->id,
                ...
                // Apply filters to all other results dynamically (if no active filter)
                foreach ($filters as $key => $details) {
                    if ($activeFilterKey && $key !== $activeFilterKey) continue;

                    $results = $query->selectRaw("{$details['column']} as id, COUNT(*) as count")
                        ->groupBy($details['column'])->get();
