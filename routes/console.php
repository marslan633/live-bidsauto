...
if (config('app.app_kvm_one') === true) {
    ...
    // app(Schedule::class)->command('process:archived-data-with-elasticsearch')->cron('49 * * * *')->withoutOverlapping();
...
if (config('app.app_kvm_four') === true) {
   //  app(Schedule::class)->command('index:vehicle-records')->dailyAt('03:011')->withoutOverlapping();
   app(Schedule::class)->command('index:vehicle-records')->cron('*/11 * * * *')->withoutOverlapping();
   app(Schedule::class)->command('index:landing-page-rules')->cron('*/11 * * * *')->withoutOverlapping();
